...
DPNS_CONTRACT_BLOCK_HEIGHT=$(yarn hthmate config get --config="${CONFIG}_1" platform.dpns.contract.blockHeight)
DPNS_TOP_LEVEL_IDENTITY_ID=$(yarn hthmate config get --config="${CONFIG}_1" platform.dpns.ownerId)
DPNS_TOP_LEVEL_IDENTITY_PRIVATE_KEY=$(grep -m 1 "HD private key:" ${SETUP_FILE_PATH} | awk '{$1="";printf $5}')

FEATURE_FLAGS_IDENTITY_ID=$(yarn hthmate config get --config="${CONFIG}_1" platform.featureFlags.ownerId)
FEATURE_FLAGS_CONTRACT_ID=$(yarn hthmate config get --config="${CONFIG}_1" platform.featureFlags.contract.id)

MASTERNODE_REWARD_SHARES_CONTRACT_ID=$(yarn hthmate config:get --config="${CONFIG}_1" platform.masternodeRewardShares.contract.id)
...
echo "Mint 100 HTH to faucet address"

MINT_FILE_PATH=${PATH_TO_PROJECT_ROOT}/logs/mint.log

yarn hthmate wallet mint --verbose --config=local_seed 100 | tee "${MINT_FILE_PATH}"

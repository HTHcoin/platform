const RpcClient = require('@hthcoin/helpthehomelessd-rpc');
const HthCoreRpcError = require('../../errors/HthCoreRpcError');
const constants = require('./constants');
const config = require('../../config');

const client = new RpcClient(config.hthcore.rpc);
...
function generateToAddress(blocksNumber, address) {
  return new Promise((resolve, reject) => { // not exist?
    client.generateToAddress(blocksNumber, address, (err, r) => {
      if (err) {
        reject(new HthCoreRpcError(err.message, null, err.code));
      } else {
        resolve(r.result);
      }
    });
...
const getMnListDiff = (baseBlockHash, blockHash) => new Promise((resolve, reject) => {
  client.protx(constants.HTHCORE_RPC_COMMANDS.protx.diff, baseBlockHash, blockHash, (err, r) => {
    if (err) {
      reject(new HthCoreRpcError(err.message, null, err.code));
    } else {
      resolve(r.result);
...
const getMerkleBlocks = (bloomFilter, fromBlockHash, count) => new Promise((resolve, reject) => {
  client.getMerkleBlocks(bloomFilter, fromBlockHash, count, (error, response) => {
    if (error) {
      reject(new HthCoreRpcError(error.message));
    } else {

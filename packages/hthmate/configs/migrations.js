...
module.exports = {
  '0.17.2': (configFile) => {
    Object.entries(configFile.configs).filter(([, config]) => config.network === NETWORK_TESTNET)
      .forEach((config) => {
        // Set HthCoin contract ID and block height for testnet
        // Set seed nodes for testnet tenderhth
        lodashSet(config, 'platform.drive.tenderhth.p2p.seeds', systemConfigs.testnet.platform.drive.tenderhth.p2p.seeds);
        lodashSet(config, 'platform.drive.tenderhth.p2p.persistentPeers', []);
      });
  ...
  '0.17.3': (configFile) => {
    Object.entries(configFile.configs).filter(([, config]) => config.network === NETWORK_TESTNET)
      .forEach((config) => {
        // Set HthCoin contract ID and block height for testnet
        lodashSet(config, 'platform.hthcoin', systemConfigs.testnet.platform.hthcoin);
      });
  ...
  '0.18.0': (configFile) => {
    ...
    Object.entries(configFile.configs)
      .forEach(([, config]) => {
        ...
        lodashSet(
          config,
          'platform.drive.tenderhth.docker.image',
          systemConfigs.base.platform.drive.tenderhth.docker.image,
        );
  ...
  '0.19.0': (configFile) => {
    ...
    Object.entries(configFile.configs)
      .forEach(([, config]) => {
        ...
        if (typeof config.platform !== 'undefined') {
          // Add Tenderhth node ID
          config.platform.drive.tenderhth.nodeId = null;
          ...
          config.platform.drive.tenderhth.consensus = systemConfigs.base
            .platform.drive.tenderhth.consensus;
    ...
    configFile.configs.testnet.platform.drive.tenderhth.p2p.seeds = systemConfigs.testnet.platform
      .drive.tenderhth.p2p.seeds;
    configFile.configs.testnet.platform.drive.tenderhth.genesis = systemConfigs.testnet.platform
      .drive.tenderhth.genesis;

    configFile.configs.testnet.platform.dpns = systemConfigs.testnet.platform.dpns;
    configFile.configs.testnet.platform.hthcoin = systemConfigs.testnet.platform.hthcoin;
    ...
    Object.entries(configFile.configs)
      .forEach(([, config]) => {
        ...
        if (config.platform) {
          ...
          if (config.platform.drive.tenderhth.consensus.createEmptyBlocks.createEmptyBlocksInterval === '10s') {
            config.platform.drive.tenderhth.consensus.createEmptyBlocks.createEmptyBlocksInterval = '3m';
          }

          // Tenderhth logging levels
          if (typeof config.platform.drive.tenderhth.log === 'undefined') {
            config.platform.drive.tenderhth.log = systemConfigs.base.platform.drive.tenderhth.log;
          }

          // Remove validator set
          if (typeof config.platform.drive.tenderhth.validatorKey === 'undefined') {
            delete config.platform.drive.tenderhth.validatorKey;
          }

          // Update images
          config.platform.drive.tenderhth.docker.image = systemConfigs.base.platform
            .drive.tenderhth.docker.image;

  ...
  function renderServiceTemplates(config) {
    ...
    const templatePaths = glob
      ...
      .filter((templatePath) => {
        ...
        // Don't create blank tenderhth configs
        if (templatePath.includes('templates/platform/drive/tenderhth')) {
          const skipEmpty = {
            genesis: 'genesis',
            node_key: 'nodeKey',
          };
          ...
          for (const [configName, optionName] of Object.entries(skipEmpty)) {
            const option = config.get(`platform.drive.tenderhth.${optionName}`);

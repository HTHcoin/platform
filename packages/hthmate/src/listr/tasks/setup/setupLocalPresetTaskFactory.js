  ...
  function setupLocalPresetTask() {
    return new Listr([
          ...
          const subTasks = ctx.configGroup.map((config, i) => (
            {
              ...
              task: () => {
                ...
                if (config.getName() === 'local_seed') {
                  ...
                  config.set('platform.drive.tenderhth.p2p.port', 26656 + (i * 100));
                  config.set('platform.drive.tenderhth.rpc.port', 26657 + (i * 100));
                  ...
                  if (ctx.debugLogs) {
                    ...
                    config.set('platform.drive.tenderhth.log.level', {
                      '*': 'debug',
                    });
                  }
      ...
      },
      {
        ...
        task: (ctx) => configureTenderhthTask(ctx.configGroup),
      },
      {

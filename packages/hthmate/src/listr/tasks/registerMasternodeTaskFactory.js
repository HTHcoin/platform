    ...
    return new Listr([
      ...
      },
      {
        ...
        task: async (ctx, task) => {
          ...
          if (balance <= MASTERNODE_HTH_AMOUNT) {
            throw new Error(`You need to have more than ${MASTERNODE_HTH_AMOUNT} HTH on your funding address`);
          }
        },
      },
      {
      ...
      {
        title: 'Send 0.1 hth from funding address to reward address',
        task: async (ctx) => {
        ...
        task: async (ctx) => (
          new Observable(async (observer) => {
            await waitForBalanceToConfirm(
              ...
              ctx.reward.address,
              (balance) => {
                observer.next(`${balance} hth to confirm`);
              },
            );
            observer.complete();
      ...
      {
        title: `Send ${MASTERNODE_HTH_AMOUNT} hth from funding address to collateral address`,
        task: async (ctx, task) => {
          ctx.collateralTxId = await sendToAddress(
            ctx.coreService,
            ctx.fundingPrivateKeyString,

...
class GroupResetCommand extends GroupBaseCommand {
   ...
   * @param {configureTenderhthTask} configureTenderhthTask
  ...
  async runWithDependencies(
    ...
    const tasks = new Listr(
      [
          ...
          task: () => new Listr(configGroup.map((config) => ({
            ...
            task: (ctx) => {
              ...
              if (config.has('platform')) {
                ...
                config.set('platform.hthcoin', baseConfig.platform.hthcoin);
                ...
                config.set('platform.drive.tenderhth.nodeId', baseConfig.platform.drive.tenderhth.nodeId);
                config.set('platform.drive.tenderhth.nodeKey', baseConfig.platform.drive.tenderhth.nodeKey);
                config.set('platform.drive.tenderhth.genesis', baseConfig.platform.drive.tenderhth.genesis);
              }
        ...
        {
          enabled: (ctx) => !ctx.isHardReset,
          title: 'Configure Tenderhth nodes',
          task: () => configureTenderhthTask(configGroup),
        },
        {
        ...
        },
        {
          ...
          title: `Generate ${amount} hth to local wallet`,
          enabled: (ctx) => !ctx.isHardReset,

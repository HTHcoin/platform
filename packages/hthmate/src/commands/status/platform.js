...
class PlatformStatusCommand extends ConfigBaseCommand {
  ...
  async runWithDependencies(
    ...
    if (!(await dockerCompose.isServiceRunning(config.toEnvs(), 'drive_tenderhth'))) {
      throw new ServiceIsNotRunningError(config.get('network'), 'drive_tenderhth');
    }
    ...
    // Collecting platform data fails if Tenderhth is waiting for core to sync
    ...
    const tenderhthStatusRes = await fetch(`http://localhost:${config.get('platform.drive.tenderhth.rpc.port')}/status`);
    const {
      result: {
        node_info: {
    ...
    } = await tenderhthStatusRes.json();

    const tenderhthNetInfoRes = await fetch(`http://localhost:${config.get('platform.drive.tenderhth.rpc.port')}/net_info`);
    const {
      result: {
        n_peers: platformPeers,
      },
    } = await tenderhthNetInfoRes.json();
    ...
    try {
      ...
      const p2pPortStateRes = await fetch(`https://mnowatch.org/${config.get('platform.drive.tenderhth.p2p.port')}/`);
    ...
    try {
      ...
      } = await dockerCompose.inspectService(config.toEnvs(), 'drive_tenderhth'));
    } catch (e) {
    ...
    const outputRows = {
      'Tenderhth Version': platformVersion,
      Network: platformNetwork,
      Status: status,
      'Block height': blocks,
      ...
      'P2P service': `${config.get('externalIp')}:${config.get('platform.drive.tenderhth.p2p.port')}`,
      'P2P port': `${config.get('platform.drive.tenderhth.p2p.port')} ${p2pPortState}`,
      'RPC service': `127.0.0.1:${config.get('platform.drive.tenderhth.rpc.port')}`,
    };

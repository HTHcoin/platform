...
describe('getPlatformDefinition', () => {
  describe('v0', () => {
    it('should return loaded GRPC package definition', async () => {
      ...
      expect(platformDefinition.service.broadcastStateTransition.path).to.equal('/org.hth.platform.dapi.v0.Platform/broadcastStateTransition');

      expect(platformDefinition.service).to.have.property('getIdentity');
      expect(platformDefinition.service.getIdentity.path).to.equal('/org.hth.platform.dapi.v0.Platform/getIdentity');

      expect(platformDefinition.service).to.have.property('getDataContract');
      expect(platformDefinition.service.getDataContract.path).to.equal('/org.hth.platform.dapi.v0.Platform/getDataContract');

      expect(platformDefinition.service).to.have.property('getDocuments');
      expect(platformDefinition.service.getDocuments.path).to.equal('/org.hth.platform.dapi.v0.Platform/getDocuments');
    });
  });
});
